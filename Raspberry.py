...
import threading
import time
import calc #our personal library
...
def compute_send():
    ...
    while True:
        if len(point_queue) > 0:
            ...
            angles = calc.calculateTriangleAngles(x, y)
            angle_queue.append(angles)
    ...
    for event in pygame.event.get():
        ...
        if mouse_down and event.type == pygame.MOUSEMOTION:
            ...
            if (new_x > old_x + gap_offset or new_x < old_x - gap_offset) or (new_y > old_y + gap_offset or new_y < old_y - gap_offset):
                ...
                res = calc.calculateTriangleAngles(new_point[0], new_point[1])
                print("Angles: %d, %d, %d, %d,", (res[0], res[1], res[2], res[3]) )
